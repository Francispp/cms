 ...
 *
 */
public class ExcelEditWriter extends ComponentWriter
{
	public ExcelEditWriter()
	{
		super ("Excel", "DIV");
	}
	...
	protected void writeComponentEnd() 
	{
		...
		String src="${ctx}/cms/document!editOfficeItem.action?type=1&id=${domain.id}&optName=excelEditor"+id+"&templateUrl=";	
		...
		sb.append("<IFRAME ID='excelEditor"+id+"' src='"+src+templateUrl+"' frameborder='0' scrolling='no' width='100%' height='100%' style='height:600px'></IFRAME>");
		sb.append(" </c:if>\n");
	    sb.append(" <c:if test=\"${!empty domain."+name+"}\" >\n");
		sb.append("<IFRAME ID='excelEditor"+id+"' src='"+src+"${domain."+name+"}' frameborder='0' scrolling='no' width='100%' height='100%' style='height:600px'></IFRAME>");
		sb.append(" </c:if>\n");		
		...
		sb.append("   $(\""+name+"\").value=\""+Constants.INFO_OFFICE_PATH+"/${domain.id}/"+fileName+".xls\";\n");
		sb.append("  var iframe"+id+"=document.frames['excelEditor"+id+"'];//document.all.excelEditor"+id+";\n");
		...
		sb.append("  iframe"+id+".SaveToURL(\""+fileName+".xls\"); \n");  		
		sb.append(" }\n");
