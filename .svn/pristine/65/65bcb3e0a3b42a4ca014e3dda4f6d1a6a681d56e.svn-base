 ...
 *
 */
public class PptEditWriter extends ComponentWriter
{
	public PptEditWriter()
	{
		super ("Ppt", "DIV");
	}
	...
	protected void writeComponentEnd() 
	{
		...
		String src="${ctx}/cms/document!editOfficeItem.action?type=2&id=${domain.id}&optName=pptEditor"+id+"&templateUrl=";	
		...
		sb.append("<IFRAME ID='pptEditor"+id+"' src='"+src+templateUrl+"' frameborder='0' scrolling='no' width='100%' height='100%' style='height:600px'></IFRAME>");
		sb.append(" </c:if>\n");
	    sb.append(" <c:if test=\"${!empty domain."+name+"}\" >\n");
		sb.append("<IFRAME ID='pptEditor"+id+"' src='"+src+"${domain."+name+"}' frameborder='0' scrolling='no' width='100%' height='100%' style='height:600px'></IFRAME>");
		sb.append(" </c:if>\n");		
		...
		sb.append("   $(\""+name+"\").value=\""+Constants.INFO_OFFICE_PATH+"/${domain.id}/"+fileName+".ppt\";\n");
		sb.append("  var iframe"+id+"=document.frames['pptEditor"+id+"'];//document.all.pptEditor"+id+";\n");
		...
		sb.append("  iframe"+id+".SaveToURL(\""+fileName+".ppt\"); \n");  		
		sb.append(" }\n");
