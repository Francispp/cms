...
public class HomewaplistWriter extends ComponentWriter{
	public HomewaplistWriter()
	{
		super("Homewaplist","");
	}
	@Override
	...
	protected void writeComponentStart(StartElementToken startElement)
	{
		...
		titleEnd = !StringUtil.isEmpty(titleEnd)? StringEscapeUtils.unescapeHtml(titleEnd) : "";
		...
		if(!StringUtil.isEmpty(infotype) && infotype.equals("2"))
			 jsp.addln("Page datas = documentCommonService.findByPublishPageSiteDocs (limit, criteriaSetup, where, null,channel,2);");
		else if(!StringUtil.isEmpty(infotype) && infotype.equals("1")) 
			 jsp.addln("Page datas = documentCommonService.findChildByPublishPage (limit, criteriaSetup, channel, where,2);");
		else
			 jsp.addln("Page datas = documentCommonService.findByPublishPage (limit, criteriaSetup, channel, where,2);");
