...
<%@ include file="/common/meta.inc" %>
    ...
    <!--link rel="stylesheet" href="${ctx}/styles/dtree.css" type="text/css"--><%@ include file="/common/cxtree.inc" %>
  ...
  <body class="pw_ab" style="overflow:hidden;padding:0px;margin:0px;border:0px;">
    ...
    <form method="post" action="permission!saveOrUpdate.action" name="myform" class="pw-borderOut_ab pw-w417_ab">
      <ww:hidden name="type" id="type"/><ww:hidden name="objectid" id="objectid"/><ww:hidden name="keys" id="keys" value=""/>
      <div class="pw-borderIn_ab" style="height:400px;">
        <div class="pw-head_ab">
          <div class="pw-head-tit_ab">
            <img src="${default_imagepath}/ico_013_plus.gif" class="ico_ab ico-013_ab" /><span>赋权</span>
          </div>
          <img src="${default_imagepath}/pic_popwindow_close.jpg" class="pwClose_ab" id="pwClose_ab" />
        </div>
        <div class="pw-con_ab" >
          <ul class="pw-con-input_ab" style="height:300px;margin-left:27px; overflow-y:auto">
            <li class="pw-con-input-item_ab">
              <input type='checkbox' name='allbox' onclick='checkAll(this);'/>
              全选
            </li>
            <li class="pw-con-input-item_ab">
              <c:forEach var="resource" items="${requestScope.resourse}" varStatus="rstatus">
                <input type="Checkbox" name="selCheckObj" onclick='checkOne(allbox);' id ="${resource.resourceid}" value="${resource.resourceid}" />
