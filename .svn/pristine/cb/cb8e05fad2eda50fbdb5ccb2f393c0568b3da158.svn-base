...
<script type="text/javascript">			
 ...
 function editItem(oid){
  location.href='regular!edit.action?id='+oid+parameterUrl;
 }
 //新增
  function addItem(){
  location.href='regular!edit.action?id='+parameterUrl;
 }
 //删除
 function deleteItem(){
 ...
 if(confirm('您确定要删除选择的记录吗？')){
  location.href='regular!delete.action?keys='+ids+parameterUrl;
 }
 }
</script>
<style type="text/css">
...
<ec:table
	items="items" var="item" action="${ctx}/crawl/regular!list.action"
	updateAction="${ctx}/crawl/regular!saveAjax.action"
	deleteAction="${ctx}/crawl/regular!deleteAjax.action"
	insertAction="${ctx}/crawl/regular!saveAjax.action" editable="false"
	...
	<ec:row recordKey="${item.oid}" rowId="rowid_${GLOBALROWCOUNT}">
		...
		<ec:column property="title" title="名称" width="10%" sortable="true"
			filterable="true" editTemplate="ecs_t_input"
			value="<a href='#' onclick='editItem(${item.oid})'>${item.title}</a>" />
			
			<ec:column property="content" title="表达式" width="*%" sortable="true"
			filterable="true" editTemplate="ecs_t_input"
			value="<a href='#' onclick='editItem(${item.oid})'><xmp>${item.content}</xmp></a>" />
	</ec:row>
