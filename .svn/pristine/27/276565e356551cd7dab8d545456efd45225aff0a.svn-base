...
Me.initWaitingBar=function(){
	...
	Me.waitingBar.style.left="50%";//wLeft+"px";
...
ECSideUtil.doExport=function(type,fileName,page,formid){
	...
	if(type=='print'){
		page=!confirm(ECSideMessage.PRINT_CONFIRM);
	}else{
...
ECSideUtil.NearPagesBar.initMe=function(){
	...
	if (!temp){
		...
		if(document.getElementById("myTable_toolbar") != null)
		document.getElementById("myTable_toolbar").appendChild(this.coreElement);
...
ECSideUtil.doFilter=function(event,filterInputObj,filterFieldName,formid,isDateField){
	var e = event||window.event;
	...
	if(isDateField){
		ECSideUtil.doFilterCore(filterInputObj,filterFieldName,ECSideConstants.FILTER_ACTION,formid,true);
	} else {
...
ECSideUtil.doFilterCore=function(filterInputObj,filterFieldName,faction,formid,isDateField){
...
if (filterInputObj!=null){
	if(isDateField){
		form[filterFieldName].value=document.getElementById(filterFieldName+"_start").value+","+document.getElementById(filterFieldName+"_end").value;
	} else {
...
ECSideUtil.ColmunMenu.getFilterItem=function(menuWidth,columnObj,columnName,formid){
	...
	var isDateField=columnObj.getAttribute("isDateField");
	var isDate = false;
	var template=document.getElementById(templateId);
	var templateText=window.isIE?template.value:template.textContent;
	templateText=ECSideUtil.trimString(templateText);

	if(isDateField&&isDateField=="1"){
		var cf = "WdatePicker({vel:this,dateFmt:'yyyy-MM-dd',errDealMode:1})";
		jsfunction="ECSideUtil.doFilter(event,this,'"+filterFieldName+"','"+formid+"',true)";
		var values = v.split(',');
		if(values.length < 2){
			values = ['',''];
		}
		filterHTML="<nobr><input type=\"button\" class=\"filterIcon\" onclick=\""+jsfunction+"\" />从:<input type=\"text\" id=\""+filterFieldName+"_start\" class=\"filterInput\" value=\""+values[0]+"\" style=\"width:"+(menuWidth- FIX_WIDTH)+"px\" onclick=\"ECSideUtil.doFilterFocus(event,this)\" onfocus=\""+cf+"\" onkeypress=\""+jsfunction+"\" /></nobr><br/><nobr><input type=\"button\" class=\"filterIcon\" style=\"background:none;\" />至:<input type=\"text\" name=\""+filterFieldName+"_end\" class=\"filterInput\" value=\""+values[1]+"\" style=\"width:"+(menuWidth- FIX_WIDTH)+"px\" onclick=\"ECSideUtil.doFilterFocus(event,this)\" onfocus=\""+cf+"\" onkeypress=\""+jsfunction+"\" /></nobr>";
		...
		var cf = '';
		var st = templateText.toLowerCase().indexOf(" onfocus=")+9;//必需写成demo: onfocus="acb()"
		if(st>8){
			try{
				cf = templateText.substring(st+1);
				cf = cf.substring(0,cf.indexOf('"'));
			}catch(e){
				cf = '';
			}
		}
		filterHTML+="<input type=\"text\" class=\"filterInput\" value=\""+v+"\" style=\"width:"+(menuWidth- FIX_WIDTH)+"px\" onclick=\"ECSideUtil.doFilterFocus(event,this)\" onfocus=\""+cf+"\" onkeypress=\""+jsfunction+"\" /></nobr>";
