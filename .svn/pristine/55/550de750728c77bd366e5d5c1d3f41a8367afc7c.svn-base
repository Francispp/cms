 ...
 <c:forEach var="site" items="${sites}" varStatus="status">
  ...
  var docPer='${docPer}';
   if(isNaN(value)){
     ...
     {
    if(docPer == "true")
    	 myUrl='${ctx}/cms/permission.action?type=1&objectType=3&objectId='+pid+"&comstatus=${status}";
     else
         myUrl='${ctx}/cms/permission.action?type=1&objectType=1&objectId='+pid+"&comstatus=${status}";
   
     }
     top.setMainFrameUrl(myUrl);
   }else{
    // myUrl='${ctx}/cms/channel!comTabbar.action?id='+value+"&comstatus=${status}";
    if(docPer == "true")
    	 myUrl='${ctx}/cms/permission.action?type=2&objectType=3&objectId='+value+"&comstatus=${status}";
      else	 
         myUrl='${ctx}/cms/permission.action?type=2&objectType=2&objectId='+value+"&comstatus=${status}";
...
//新增站点
function addSite(){
 window.parent.parent.frames['main_frame'].location.href='${ctx}/cms/site!comTabbar.action?comstatus=${status}';
}
...
function addChanel(value){
  ...
  window.parent.parent.frames['main_frame'].location.href="${ctx}/cms/channel!comTabbar.action?siteid="+siteid+"&pchannelid="+pchannelid;
...
<!--		
  ...
  tree${site.oid}.loadXML("${ctx}/cms/site!xml.action?id=${site.oid}&siteName="+encodeURI(encodeURI("站点")));
