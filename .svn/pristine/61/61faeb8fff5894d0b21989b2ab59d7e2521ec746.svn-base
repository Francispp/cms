...
function WebFXLoadTree(sText, sXmlSrc,sAction, sBehavior, sIcon, sOpenIcon,sRadio,sCheckBox,sValue,sChecked,sSelectClick,sOnMouseup) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon,sRadio,sCheckBox,sValue,sChecked,sSelectClick,sOnMouseup)
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon,sRadio,sCheckBox,sValue,sChecked,sSelectClick,sOnMouseup) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon,sRadio,sCheckBox,sValue,sChecked,sSelectClick,sOnMouseup);
...
function _xmlTreeToJsTree(oNode) {
	...
	var onmouseup= oNode.getAttribute("onMouseup");
        var sSelectClick=oNode.getAttribute("selectClick");//"setResult("+sid+",'"+text+"')";
	...
	var jsNode;
	if (src != null && src != "")
	//(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon,sRadio,sCheckBox,sValue,sChecked,sSelectClick,sOnMouseup)
		jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon,sRadio,sCheckBox,sid,sChecked,sSelectClick,onmouseup);
	else
		//jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon,sid,onMouseup);
		jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon,sRadio,sCheckBox,sid,sChecked,sSelectClick,onmouseup);
		...
		for (var i = 0; i < l; i++) {
			if (cs[i].tagName == "tree") {
				...
				jsParentNode.add( _xmlTreeToJsTree(cs[i]), true);
                 ...
                 //alert(oXmlDoc.documentElement.length);
                 //oXmlDoc.documentElement=null;
