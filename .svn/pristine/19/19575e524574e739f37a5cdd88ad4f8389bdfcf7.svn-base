	...
	response : function() {
		if (this.transport.status == '200') {
			...
			if (data.indexOf("xmlns:burlap") == -1) {
				...
				reply = new Buffalo.Reply(data);
			} else {
				reply = new Buffalo.Reply(this.transport);
			}
...
Buffalo.Default = {
	...
	showLoading : function(state) {
		...
		if (this.loadingPane == null) {
			...
			el.style.cssText="display:none;font-family:ËÎÌו,verdana,Arial, Helvetica; font-size:16px; position:absolute;overflow:auto; z-index:999999; left: 0px; top: 0px;";
			el.innerHTML="<img src=\""+waitgifpath+"/common/buffalo/loading.gif\">";
			document.body.appendChild(el);
			this.loadingPane = el;
		}
		if (state) {
			this.loadingPane.style.display="block";
			this.loadingPane.style.left = document.body.clientWidth/2-70;
			this.loadingPane.style.top = document.body.clientHeight/2-30;
		} else {
	...
	showError: function(errorStr) {
		...
		if (this.errorPane == null) {
			...
			el.style.cssText="font-family:Verdana;font-size:16px;border:1px solid #00CC00;background-color:#ffdb9c;padding:1px;position:absolute;overflow:auto; left:0px; top:0px; width:800px; height:600px; z-index:1";
...
Buffalo.Reply.prototype = {
	initialize: function(sourceXML) {
		...
		if (typeof(sourceXML) == "string") {
			...
			this._source = sourceXML.responseText;
			var root = sourceXML.responseXML.documentElement;
