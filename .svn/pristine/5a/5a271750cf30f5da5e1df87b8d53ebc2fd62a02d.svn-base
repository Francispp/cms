 ...
 */
package com.cyberway.issue.crawler.writer;
...
import com.cyberway.issue.crawler.replace.Replace;
import com.cyberway.issue.crawler.replace.ReplaceURL;
import com.cyberway.issue.crawler.settings.ListType;
...
import com.cyberway.issue.crawler.settings.StringList;
 ...
 * <ul>
 * <li> URIs can have arbitrary length, but file systems have length
 * constraints. </li>
 * <li> URIs can contain characters that file systems prohibit. </li>
 * <li> URI paths are case-sensitive, but some file systems are
 * case-insensitive. </li>
 * </ul>
 ...
 */
public class MirrorWriterProcessor extends Processor implements
	...
	protected void innerProcess(CrawlURI curi) {
		...
		try {
			...
			String filePath = "";
			boolean isRepleaceURL = false;
			if (mimeType != null
					&& (mimeType.toLowerCase().indexOf("css") != -1)
					&& (curi.toString().toLowerCase().endsWith(".css"))) {
				filePath = "css";
			} else if (curi.toString().toLowerCase().indexOf("/images") != -1
					&& ((curi.toString().toLowerCase().endsWith(".jpg"))
							|| (curi.toString().toLowerCase().endsWith(".gif"))
							|| (curi.toString().toLowerCase().endsWith(".png")) || (curi
							.toString().toLowerCase().endsWith(".swf")))) {
				filePath = "images";
			} else if (curi.toString().indexOf("docInfo!list.action") > 0
					&& curi.toString().indexOf("&docid=") == -1) {
				filePath = "summary_template";
				isRepleaceURL = true;
			} else if (curi.toString().indexOf("docInfo!view.action") > 0
					|| curi.toString().indexOf("&docid=") > 0) {
				filePath = "details_template";
				isRepleaceURL = true;
			} else if (curi.toString().indexOf("index.action") > 0) {
				filePath = "\\";
				mps = mps.substring(0, mps.lastIndexOf('\\') + 1)
						+ "index.html";
				isRepleaceURL = true;
			}
			System.out.println(curi.toString() + "-----------");
			if (StringUtils.isNotBlank(filePath)) {
				if (curi.toString().indexOf("&url=") > 0) {
					mps = mps.substring(0, mps.lastIndexOf('/'))
							+ mps.substring(mps.lastIndexOf('/') + 1, mps
									.length());

				}
				mps = mps.replace(".action", ".html");
				File file = new File(baseDir + File.separator
						+ mps.substring(0, mps.indexOf("\\")) + File.separator
						+ filePath + File.separator
						+ mps.substring(mps.lastIndexOf("\\", mps.length())));
				...
				try {
					if (isRepleaceURL) {
						Replace[] replace = new Replace[1];
						replace[0] = new ReplaceURL();
						String content = replace[0].Replace(recis, uuri);
						createFile(destFile.getPath(), content);
					} else {
			...
			/*
			 * ReplayInputStream inputStream =
			 * recis.getContentReplayInputStream(); byte[] bytes = new
			 * byte[(int)inputStream.getSize()]; inputStream.read(bytes);
	...
	private static String ENCODING = "GB2312";// "UTF-8";
	...
	public static void createFile(String output, String content)
		...
		try {
			fw = new OutputStreamWriter(new FileOutputStream(output), "utf-8");
	...
	private void writeToFile(RecordingInputStream recis, File dest)
			throws IOException {
		ReplayInputStream replayis = recis.getContentReplayInputStream();
		File tf = new File(dest.getPath() + "N");
		FileOutputStream fos = new FileOutputStream(tf);
