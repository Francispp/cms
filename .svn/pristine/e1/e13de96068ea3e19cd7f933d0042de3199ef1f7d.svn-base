...
<ww:hidden name="type" id="type"></ww:hidden>
...
<ul id="lion-ul-a">
  ...
  <div class="edit-header-bottom" style="margin-top:10px;">
    ...
    <li style="margin-right:20px;">&nbsp;&nbsp;&nbsp;访问时间 </li>
...
<input type="text" name="enddate" value="<ww:date name="enddate" format="yyyy-MM-dd"/>" id="enddate" onclick="WdatePicker({el:$dp.$('enddate'),dateFmt:'yyyy-MM-dd', minDate:'#F{$dp.$D(\'startdate\')}'})" class="date">
      ...
      <li style="margin-left: 10px">统计类别&nbsp;&nbsp;&nbsp; </li>
       ...
       <c:if test="${visittype==3}"><option value="3" selected="selected">按站点统计</option></c:if>
        <c:if test="${visittype!=3}"><option value="3" >按站点统计</option></c:if>
...
<ec:table items="items"
	      var="item" 
	      action="${ctx}/base/visit!listbysite.action"
