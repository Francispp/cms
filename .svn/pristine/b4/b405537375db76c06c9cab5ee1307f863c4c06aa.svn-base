...
<ww:hidden name="type" id="type" value="2"></ww:hidden>
...
<input type="text" name="enddate" value="<ww:date name="enddate" format="yyyy-MM-dd"/>" id="enddate" onclick="WdatePicker({el:$dp.$('enddate'),dateFmt:'yyyy-MM-dd', minDate:'#F{$dp.$D(\'startdate\')}'})" class="date">
      ...
      <li style="margin-left: 10px">统计类别 </li>
       <li ><select id="visittype" onchange="selectChoose()">
       ...
       <option value="1">按文档统计</option>
        <c:if test="${visittype==2}"><option value="2" selected="selected">按频道统计</option></c:if>
        <c:if test="${visittype!=2}"><option value="2" >按频道统计</option></c:if>
...
items="items"
	          var="item" 
	          action="${ctx}/base/visit!listbychanel.action"
