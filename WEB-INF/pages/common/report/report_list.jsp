...
<ec:table
	...
	<ec:row recordKey="${item.reportid}" rowId="rowid_${GLOBALROWCOUNT}">
		<ec:column property="reportid"  width="10%"
			title="<input type='checkbox' name='allbox' onclick='checkAll(this);' />全选"
			style="text-align:center">
			<input type="checkbox" name="_selectitem" value="${item.reportid}"
				onclick='checkOne(allbox);' />
		</ec:column>
		<ec:column property="_1" title="序号" width="10%" sortable="false"
			editable="false" filterable="false"
			value="<a href='#' onclick='editItem(${item.reportid})'>${GLOBALROWCOUNT}</a>"
			style="text-align:center" />
		<ec:column property="reportName" title="报表名" width="40%"  	sortable="true" 
			 value="<a href='#' onclick='editItem(${item.reportid})'>${item.reportName}</a>"  />
		<ec:column property="reportCode" title="报表编码" width="*%" 	sortable="true" 
		  	 />  
