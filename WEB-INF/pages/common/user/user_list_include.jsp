...
function doReturn()
{
	...
	obj[0]='${roleid}';
	obj[1]=''+t;
	ExecuteService("if(reply.getResult()=='true'){alert('增加用户操作成功!');window.close();}else alert('增加用户操作失败!');",'roleManagerService','insertUsersToRole',obj);
...
function selectUser(key)
{
   if(key=='')
   {
	...
	var url="${ctx}/base/user!searchUser.action?keys=" + key;
...
<form name="formItem">
<xml id="myXML"></xml>
<table width="100%" border="0" height=450  cellpadding="4" cellspacing="2" bgcolor="#C4E1FF">
<tr >
<td   bgcolor="#FFFFFF" width='62%'  valign="top">
...
<img style="CURSOR:HAND;" src="${ctx}/images/cms/btn_del.gif" border=0 onClick="try{_selectedItem.remove(formItem._selectedItem.selectedIndex);}catch(e){}">
  ...
  <tr>
   <td>   
    <select name="_selectedItem" style="width:100% " size=26 ondblClick='try{_selectedItem.remove(formItem._selectedItem.selectedIndex);}catch(e){}'></select>
   </td>
  </tr>
</table>
</td>
...
if(userOnly=='')
{
divUser.innerHTML="<select name=_selectedUser  size='10' style='width:100%' multiple></select>";
}
else
{
divUser.innerHTML="<select name=_selectedUser  size='25' style='width:100%' multiple></select>";
}
divGroup.innerHTML="<select name=_selectedGroup  size='10' style='width:100%' multiple></select>";
var id='${selected}';
