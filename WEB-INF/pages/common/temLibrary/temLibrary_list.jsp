...
<c:set var="title" value="模版库分类管理列表" /> 
...
<script type="text/javascript">
	...
	function editItems(oid){
		global_ab.fn.popWindow.addPopWindow('base/temLibrary!editCommonType.action?id='+oid ,  "500px", "175px", false);
	}

	function add() {
		global_ab.fn.popWindow.addPopWindow("base/temLibrary!editCommonType.action", "500px", "175px", false);
	}
	...
	function edit() {
		...
		global_ab.fn.popWindow.addPopWindow("base/temLibrary!editCommonType.action?id=" + ids[0], "500px", "175px", false);
	}
		...
		commonInfoService.dwrCoreCommonInfosByCommonTypeId(keys,
			function(data){
				if (confirm(data + " 确定删除所选记录吗?")) {
					location.href = "${ctx}/base/temLibrary!deleteCommonType.action?keys=" + keys;
...
<div class="content">
	<div class="info-box">	
		<ec:table
			items="items" var="item" action="${ctx}/base/temLibrary!listCommonType.action" 
			...
			<ec:row recordKey="${item.id}" rowId="rowid_${GLOBALROWCOUNT}"> 
				...
				<ec:column property="name" title="模版库名" width="10%" sortable="true" filterable="true" editTemplate="ecs_t_input" />
