...
<script type='text/javascript' src='/dwr/interface/CommonTypeService.js'></script>
...
<script type="text/javascript">
	function save(){
	  	...
	  	var keyword = document.getElementById("domain.keyword").value;
		var valid = new Validation('myform',{immediate:true});
		if(valid.validate()){
			CommonTypeService.dwrCoreCommonTypeValidateKeywordUnique(id,keyword,
				function(data){ 
					if (data == "0") {
						alert("类别名重复,必须唯一!");
						document.getElementById("domain.keyword").focus();
						return;	
					}
		  			myform.submit();
				} 
...
<body class="pw_ab" style="overflow:hidden;padding:0px;margin:0px;border:0px;">
    ...
    <form name="myform" method="post" class="pw-borderOut_ab pw-w417_ab" action="${ctx }/base/commontype!saveOrUpdateCommonType.action">
        ...
        <div class="pw-head_ab">
          <div class="pw-head-tit_ab">
            <img src="${default_imagepath}/ico_013_plus.gif" class="ico_ab ico-013_ab" /><span>
              <ww:if test="domain.id!=null">编辑常用类别</ww:if>
              <ww:else>新建常用类别</ww:else>
            </span>
          </div>
          ...
          <ul class="pw-con-input_ab">
            <li class="pw-con-input-item_ab">
              <label class="lab-pw_ab">
                <img src="${default_imagepath}/ico_003_must.gif" class="ico_ab ico-003_ab" /><span class="lab-pw-tit_ab">类别名：</span>
                <ww:textfield id="domain.keyword" name="domain.keyword" cssClass="textField_ab textField-w191_ab required max-length-24 "></ww:textfield>
              </label>
            </li> 
            <li class="pw-con-input-item_ab">
              <label class="lab-pw_ab">
