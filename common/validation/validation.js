...
Validator.messagesSourceEn = [
	...
	['validate-not-equals','Not the same as with the original password.'],
...
Validator.messagesSourceCn = [
	...
	['validate-not-equals','不能与原密码一样,请重新输入'],
	['less-than','应该小于前面的值'],
	['great-than','应该大于前面的值']
]
...
Object.extend(Validation, {
	validate : function(elm, options){
		...
		cn.all(function(value) {
			var test = Validation.divHidden(value,elm,options.useTitle);
...
Validation.addAllThese([
	...
	['validate-select', Validator.messages['validate-select'], function (v,elm) {
				var vi = elm.options[elm.selectedIndex].value;
				if(vi==undefined || vi==''){
					return false;
				}
				return true;
...
Validation.addAllThese([
	...
	['validate-phone', Validator.messages['validate-phone'], function(v) {
				return Validation.get('IsEmpty').test(v) || (/^((0[1-9]{3})?(0[12][0-9])?[-])?\d{6,8}$/.test(v) || (/(^0?[1][3-9][0-9]{9}$)/.test(v)));
			}],

	['validate-mobile-phone', Validator.messages['validate-mobile-phone'], function(v) {
				return Validation.get('IsEmpty').test(v) || (/(^0?[1][3-9][0-9]{9}$)/.test(v));
			}],
	/**
	...
	['validate-not-equals',Validator.messages['validate-not-equals'], function(v,elm,args,metadata) {
				return Validation.get('IsEmpty').test(v) || $F(args[0]) != v;
			}],
	/**
	...
	['max-length',Validator.messages['max-length'],function(v,elm,args,metadata) {
		...
		//alert(strlen(v)/2);
                //return Validation.get('IsEmpty').test(v) || v.length <= parseInt(args[0]);
		return Validation.get('IsEmpty').test(v) || strlen(v) <= parseInt(args[0]);
	}],
	/*
