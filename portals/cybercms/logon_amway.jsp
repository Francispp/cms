...
function placeBarLink(str) {
		window.open(str,'newwindow','top=0,left=0,resizable=yes,toolbar=yes,menubar=yes,location=yes,scrollbars=yes,status=yes');
	}
	function mOvr(src) 
  {
...
function setLanguage(language,style){
 ...
 location='logon!language.action?language='+language+'&style='+style;
}        
</script>
</head> 
...
<p>
 <form method="post" action="${ctx}/logon!logon.action" name="myform">
...
<table width="70%" cellspacing="2" cellpadding="0" align="left" border="0">
    ...
    <table cellspacing="2" cellpadding="0" align="left" border="0">
    		...
    		<input autocomplete="off" dir="ltr" type="text" size="16"  name="loginid" <c:if test="${not empty param.login_error}">value='<%= session.getAttribute(AuthenticationProcessingFilter.ACEGI_SECURITY_LAST_USERNAME_KEY) %>'</c:if>/>
            ...
            <tr>
                <td class="wpsFieldText" align="left"><input autocomplete="off" onKeydown='if(event.keyCode==13){if (doSubmit()) myform.submit();}'; dir="ltr" type="password" size="16" name="password"/></td>
            </tr>
	</table>
		</td>
...
<c:if test="${not empty actionMessages}">
      <c:forEach var="msg" items="${actionMessages}">alert("${msg}");</c:forEach>
    </c:if>
   <c:if test="${not empty actionErrors}">
    <c:forEach var="err" items="${actionErrors}">alert("${err}");</c:forEach>
   </c:if>
</script>
